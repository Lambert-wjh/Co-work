...
public class DAO {
  ...
  public List<Team> createTeams(String select_clause, List<String> parameters) {
    ...
    try (Connection connection = this.data_source.getConnection()) {
      try (PreparedStatement prepared_statement = connection.prepareStatement(select_clause)) {
        ...
        try (ResultSet result_set = prepared_statement.executeQuery()) {
          while (result_set.next()) {
            ...
            Leader leader =
                (Leader)
                    createStaff("SELECT * FROM Employee WHERE id=?", List.of(leader_id)).get(0);
            List<Employee> employees =
                createStaff("SELECT * FROM Employee WHERE leader_id=?", List.of(leader_id));
        ...
        try (ResultSet result_set = prepared_statement.executeQuery()) {
          while (result_set.next()) {
            ...
            switch (position) {
              case EMPLOYEE -> {
                employees.add(
                    new Employee(id, name, sex, age, leader_id, position, sales, salary, password));
              }
              case LEADER -> {
                employees.add(
                    new Leader(id, name, sex, age, leader_id, position, sales, salary, password));
              }
              case SUPERUSER -> {
                employees.add(
                    new Superuser(
  ...
  public int updateTable(String update_clause, List<String> parameters) {
    ...
    try (Connection connection = this.data_source.getConnection()) {
      try (PreparedStatement prepared_statement = connection.prepareStatement(update_clause)) {
        for (int i = 0; i < parameters.size(); i++) {
          prepared_statement.setObject(i + 1, parameters.get(i));
        }
